# Dealing with Database Skew and Concurrency
...
When you interleave concurrent reads and writes to your database the results can be surprising. The results can also break your app. Let's prevent that from happening! All of these common query patterns can create concurrency headaches:
- Batch inserting or updating multiple records.
- Enforcing a constraint when reading data across the same or related tables.
- Long running analytic queries.
- Using application logic to update a value.
- Booking or reserving some resource if it's available.

I'm sure some look familiar. Some might look innocuous. Database transactions give a false sense of security that these operations will just work. But what actually happens when two transactions occur concurrently? Depending on your [isolation level](https://en.wikipedia.org/wiki/Isolation_(database_systems)) a number of phenomena are possible that will haunt your application (TODO: use a less corny word than haunt?).

## Dirty reads and dirty writes
Dirty reads and writes are the easiest phenomena to grasp. They occur when a concurrent database transaction reads or writes uncommitted data to the database.
...
COMMIT;
```
How harmless, you think. How could inserting data possibly cause issues? If the database doesn't enforce isolation between transactions, the newly written records are available in the table before the transaction has finished. These _uncommitted_ updates may be read by other transactions (dirty read). If the transaction writing the data fails, all inserts will be rolled backed. Any transaction that previously read these rows will be holding invalid data that no longer exists in the table.

Dirty writes stem from the same lack of isolation. For the bulk update, a second transaction overwrites one of the values just written by the first. When the first transaction commits, we would expect all of its changes to appear in the table atomically. In fact, some of the data written actually originates from the second ongoing transaction.

Assuming _zero_ isolation protection, this risk is still present (albeit small) for a multi-row insert/update:
```sql
...
VALUES
    ...
    (3, 'baz');
```
The database cannot magically insert all the data instantaneously and commit. Data may be scattered across multiple pages in memory. Some rows will be inserted or updated before others. Only once all rows in all pages have been updated will the database commit the result. Don't let the declarative SQL syntax fool you.

### How to ensure consistency
In practice, you don't need to worry about dirty reads/writes when using a popular RDBMS. All use a default isolation level of **READ COMMITTED** at a minimum. As the name implies, this ensures transactions can only read _committed_ data, which can't be rolled back. In Postgres, dirty reads are impossible because **READ COMMITTED** is the lowest isolation level.
...
The issue above is that a JOIN between two tables doesn't occur instantaneously. This is also an issue for self-joins on a single table. The underlying reason is that rows may be distributed across different pages, which will be read in individually if these blocks are not co-located on disk.
...
| **T3** | It reads in the spending account row, located in page Y:<br>`(id=5123, customer_id=123, type='spending', balance=200)` | |
...
| **T6** | Database reads in the savings account row, located in page Z:<br>`(id=7424, customer_id=123, type='savings', balance=900)` | |
...
We don't usually increment counters in the application code these days. Any popular RDBMS can update integers atomically, which avoids this race condition. But there are some data types that must be updated in application code. An object serialised as anything other than JSON or XML (e.g. Protobufs, Avro) will be stored as a binary large object (`BLOB`). To update these objects, we must first read and deserialise them from the database. This risks a lost update because two concurrent transactions may read and update the same record.

#### Updating a value that requires application logic
Another reason data is read into the application is when an update is derived from business logic. This makes the data vulnerable to lost updates.
...
### How to guard against it
In both the examples we've seen, there is a common read-write-update cycle. If possible, avoid this pattern and favour atomic updates. There are a few alternatives when this isn't possible. One neat solution is to perform a compare-and-set during the update. We compare the original value we read to the value currently held in the database. If the two are different, another transaction has since updated the resource and we abort or restart the operation. Another approach is to pessimistically lock the rows you are updating with a `SELECT ... FOR UPDATE`, preventing concurrent transactions from altering the data between queries. There is a penalty cost with both of these options, though can be negligible.

## Write skew
Read skew is when the application reads inconsistent data. Write skew is when the application writes to a database and leaves it in an inconsistent state. The traditional definition describes when two transactions write to different objects, and don't observe the effects of each other. This is best illustrated with an example.
...
-- Application checks the total balance won't be negative after the
-- withdrawal, then continues.
UPDATE account SET balance = balance - 150 WHERE account_id = 567
```

| Time | Transaction 1 (User withdraws money) | Transaction 2 (Direct debit) |
| :- | :------------- | :------------- |
| **T1** | `BEGIN;` | |
| **T2** | Query `SELECT SUM(balance)...` to get customer's net balance. Currently at $250. | |
| **T3** | Verify withdrawal of $100 won't cause the total balance to be negative. | |
| **T4** | | `BEGIN;` |
| **T5** | | An automated direct debit removes $200 from one of the customer's accounts. |
| **T6** | | `COMMIT;` The net balance is now $50 |
| **T7** | Withdraw $100 from account. | |
| **T8** | `COMMIT;` The net balance is now -$50 | |


The withdrawal should have been blocked. This is the example used by Berenson et al (1995) when critiquing the ANSI SQL phenomena and isolation levels.
...
Updates to balances are still effectively serialised, because they are all trying to update the same row in `customer_holdings`, but at least we are not blocking on all updates of the customer's accounts (or on all reads if the only alternative is **SERIALIZABLE** isolation).
...
Who would have thought databases could be so complicated? In truth, the phenomena discussed here are not inherent to databases, the data they store, or the world that data are born from. These phenomena come about through developers and the decisions they make when designing their systems.

In all the above cases, there are technological solutions, such as locking or increasing isolation. But there are simpler alternatives. Improved data modelling can avoid certain issues altogether, or limit the strength of their effect. First, try to reframe the problem as a fault in the app design. Understand what your users need, the journeys they go through, and build the system accordingly. Only reach for pessimistic locking as a last resort. Continue to learn about these database intricacies and you can design for scale and success!
